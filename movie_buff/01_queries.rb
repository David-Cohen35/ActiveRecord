...
def harrison_ford
  ...
  Actor.joins(:movies).where(name: 'Harrison Ford').select('movies.id', :title).where.not( castings: {ord: 1})
end
...
def biggest_cast
  ...
  # Actor.joins(:movies).select('movies.id', :title).group()#.order('COUNT(actors.id) DESC').limit(3)
end
...
def directed_by_one_of(them)
  ...
  Movie.join(:actors).select(:id, :title).where()
end
